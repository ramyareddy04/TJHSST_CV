...
int threshold = 425, threshold2 = 282;
...
void sobel_conv_two(vector<vector<int>> v)
{
    ...
    for (int i=0; i<(int)v.size(); i++) {
        ...
        if (pow(tempX, 2) + pow(tempY, 2) > pow(threshold2, 2)) { // atan2(tempY, tempX)*180/M_PI
            idx += 1;
            if (pow(tempX, 2) + pow(tempY, 2) > pow(threshold, 2)) {
...
void sobel_conv_three(vector<vector<int>> v)
{
    ...
    for (int i=0; i<(int)v.size(); i++) {
        ...
        if ((row<h-1) and (row>=1) and (col<w-1) and (col>=1)) {
            ...
            if (pow(tempX, 2) + pow(tempY, 2) > pow(threshold2, 2)) {
                if (pow(tempX, 2) + pow(tempY, 2) > pow(threshold, 2)) {
                    g.illuminate(row, col, 3);
                } else {
                    g.illuminate(row, col, 2);
            ...
            temp = atan2(tempY, tempY)*180/M_PI;
            if (temp < 0) {
                    temp += 180;
            }
            nms.illuminate(row, col, round_to_multiple(temp));
            mag.illuminate(row, col, pow(tempX, 2)+pow(tempY, 2));
        } else {
            g.illuminate(row, col, 0);
        }
    }
    g.create_file("image1.ppm", 1);
    ...
    for (int i=1; i<h-1; i++) {
        for (int j=1; j<w-1; j++) {
            if (nms.get_coord(i, j)==45) {
                if (mag.get_coord(i, j) <= mag.get_coord(i+1, j-1) or mag.get_coord(i, j) <= mag.get_coord(i-1, j+1)) {
                    nms.illuminate(i, j, 0);
                }
            } else if (nms.get_coord(i, j)==135) {
                if (mag.get_coord(i, j) <= mag.get_coord(i-1, j-1) or mag.get_coord(i, j) <= mag.get_coord(i+1, j+1)) {
                    nms.illuminate(i, j, 0);
                }
            } else if (nms.get_coord(i, j)==90) {
                ...
                if (mag.get_coord(i, j) <= mag.get_coord(i, j-1) or mag.get_coord(i, j) <= mag.get_coord(i, j+1)) {
                    ...
                    nms.illuminate(i, j, 1);
                }
            }
        }
    }
    nms.create_file("image2.ppm", 1);
    
    for (int i=1; i<h-1; i++) {
        for (int j=1; j<w-1; j++) {
            if (g.get_coord(i, j)==2 or nms.get_coord(i, j)==0) {
                g.illuminate(i, j, 0);
            }
        }
    }
    g.create_file("imagef.ppm", 1);
}
