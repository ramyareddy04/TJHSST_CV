...
void write_to_video(VideoWriter video, vector<HomogeneousPoint> points, int scaleFactor, string type)
{
    ...
    while (i != 360)
    {
        ...
        for (int j=0; j<points.size(); j++) {
            HomogeneousPoint temp = translation(perspective(rotation(points[j], i, i, 0), 100, 200, 0, 0), 0, 400, 300);
            for (int k=j+1; k < points.size(); k++) {
                if (abs(points[j].sqrd_distance(points[k])-side) < 1 ) {
                    HomogeneousPoint temp2 = translation(perspective(rotation(points[k], i, i, 0), 100, 200, 0, 0), 0, 400, 300);
                    line(frame, Point(temp.getZ(), temp.getY()), Point(temp2.getZ(), temp2.getY()), Scalar(255, 255, 255), 1, 8);
                }
            }
...
void part1 () 
{
    ...
    VideoWriter video("rotation.avi", cv::VideoWriter::fourcc('M','J','P','G'), 35, Size(600, 800)); // should be (800,600)
    vector<HomogeneousPoint> points = {HomogeneousPoint(-1,-1,-1,1), HomogeneousPoint( 1, 1, 1,1),
                                       ...
                                       HomogeneousPoint(-1, 1,-1,1), HomogeneousPoint( 1,-1, 1,1),
                                       HomogeneousPoint(-1,-1, 1,1), HomogeneousPoint(-1, 1, 1,1)};
    write_to_video(video, points, 50, "square");
    ...
    write_to_video(video, points, 150, "tetrahedron");
    ...
    write_to_video(video, points, 75, "octahedron");
    points = {HomogeneousPoint(0,phi,1,1), HomogeneousPoint(0,phi,-1,1),
              ...
              HomogeneousPoint(1,0,-phi,1), HomogeneousPoint(-1,0,-phi,1)};
    write_to_video(video, points, 50, "icosahedron");
    points = {HomogeneousPoint(0,1/phi,phi,1), HomogeneousPoint(0,1/phi,-phi,1),
              ...
              HomogeneousPoint(-1, 1,-1,1), HomogeneousPoint( 1,-1, 1,1),
              HomogeneousPoint(-1,-1, 1,1), HomogeneousPoint(-1, 1, 1,1)};
    write_to_video(video, points, 50, "dodecahedron");
    video.release();
    myfile.close();
}
